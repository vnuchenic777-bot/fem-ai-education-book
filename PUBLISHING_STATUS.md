...
## Обновление от 2026-06-09

Выполнена первая сквозная редакционная очистка публичного Jupyter Book-контура для широкой аудитории. Вводные, архитектурные, методические, платформенные, сертификационные и публикационные страницы переведены из языка внутренних веток, технических заданий и служебных заготовок в более завершенную научно-методическую подачу.

Основные изменения:

- убраны видимые формулировки «черновая публикационная версия», «научно-публикационный потенциал», «техническое задание», «главная ветка проекта» там, где они создавали впечатление незавершенного внутреннего материала;
- раздел `book/02_dpo_course_design/final_project_model.md` заменен публичной академической главой об итоговом проекте курса ДПО; исходные структурированные данные сохранены в YAML/JSON и notebook-приложении;
- в notebook-главах скрыт исходный код по умолчанию через `hide-input`, чтобы публичная HTML-версия показывала таблицы, графики и пояснения, а не сырой программный слой;
- из публичного вывода notebook-глав убраны API-маршруты, endpoint-таблицы, backend/frontend-лексика и необработанные блоки с будущими публикационными идеями;
- технические разделы разработки платформы переписаны как обзорные главы о проектно-техническом основании, PWA-стратегии, ролях, доступе и хранении без раскрытия закрытых эксплуатационных деталей;
- юридические предупреждения о статусе документа об обучении, персональных данных и необходимости согласования с образовательной организацией сохранены.

Дополнительно проверен собранный HTML: в публичных HTML-страницах не найдены старые маркеры `Техническое задание`, `/api/`, `endpoint`, `черновая`, `TODO`, `production`, `/mnt/data` и блоки «Научно-публикационный потенциал».
...
Выполнена сквозная академическая редактура статей 01-03: первая статья обновлена до v0.5, вторая — до v0.3, третья — до v0.3. Тексты выровнены как единая серия: статья 01 задаёт рамку цифровой образовательной экосистемы, статья 02 раскрывает методологию программы ДПО, статья 03 описывает цифровой документ результата обучения с сохранением юридической осторожности.

Статья 02 обновлена до допилотной журнальной редакции v0.4. В неё добавлен дизайн исследования и пилотной проверки, планируемый контур анализа прохождения курса преподавателями в июле 2026 года, будущие группы показателей для графиков активности, оценивания, обратной связи и документа результата обучения. Машинно-читаемый план пилотных показателей сохранён в `data/branches/06_research_articles/article_02_dpo_methodology_v0_4/pilot_evaluation_plan.yaml`.

Статья 02 обновлена до v0.5: добавлен шаблон будущего раздела «Результаты пилотного запуска» с заготовками под общую характеристику пилота, графики активности, распределение оценок, анализ итоговых проектов, обратную связь преподавателей и проверку модели документа результата обучения. Структурированный шаблон будущих графиков и агрегированных полей сохранён в `data/branches/06_research_articles/article_02_dpo_methodology_v0_5/pilot_results_template.yaml`.

Для статьи 02 подготовлен воспроизводимый контур построения будущих графиков пилота. Скрипт `scripts/generate_pilot_result_charts.py` строит SVG-диаграммы по агрегированному YAML-шаблону `pilot_aggregates_template.yaml`: прогресс по модулям, активность по времени, распределение итоговых результатов, матрицу подтверждения компетенций, готовность итоговых проектов и сводку обратной связи. Статья 02 обновлена до v0.6.
...
- скрипт генерации будущих графиков пилота `scripts/generate_pilot_result_charts.py`;
...
- план пилотной проверки статьи 02 v0.4 в `data/branches/06_research_articles/article_02_dpo_methodology_v0_4/`.
- шаблон будущего раздела результатов пилота статьи 02 v0.5 в `data/branches/06_research_articles/article_02_dpo_methodology_v0_5/`.
- шаблон агрегированных данных пилота и синтетический пример для проверки генерации графиков в `data/branches/06_research_articles/article_02_dpo_methodology_v0_5/`.
...
| Статья 01. Концепция платформы | подключена как публикационная редакция v0.5 | `book/06_research_articles/article_01_platform_concept.md` |
| Статья 02. Методология ДПО | подключена как допилотная журнальная редакция v0.6 | `book/06_research_articles/article_02_dpo_methodology.md` |
| Статья 03. Цифровой документ результата обучения | подключена как публикационная редакция v0.3 | `book/06_research_articles/article_03_certification_model.md` |
...
- `scripts/validate_data.py`: OK, проверено 70 JSON и 91 YAML.
...
- Продолжить точечную академическую редактуру разделов PWA, ролей, хранения и проектно-технического основания по мере появления кода приложения и данных пилота.
...
- После пилотного запуска курса в июле 2026 года заполнить YAML-шаблон агрегированных данных статьи 02 и сгенерировать графики через `scripts/generate_pilot_result_charts.py`.
...
Продолжить проработку статьи 02 как журнальной редакции: подготовить будущий notebook-раздел интерпретации графиков пилота или перейти к визуальному шаблону раздела «Результаты».
