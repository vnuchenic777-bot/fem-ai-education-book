...
Диаграммы подключены через MyST figure-блоки и используют проектные данные из `book/05_certification/data/competencies.yaml` и `book/05_certification/implementation_spec.yaml`. Визуализации не меняют названия компетенций, не утверждают юридический статус документа и не раскрывают закрытые данные.
...
- Дополнить визуальный слой картой модулей, моделью данных платформы и при необходимости отдельными диаграммами для фонда оценочных средств.
...
Продолжить проработку серии статей: перейти к сквозной редактуре единого академического стиля всей серии или дополнить визуальный слой картой модулей и моделью данных платформы.
